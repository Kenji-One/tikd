          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 text-[12px] text-neutral-400">
                Team custom roles will be wired in the next batch.
              </div>
            </div>
          </div>
...
export default function TeamMembersPage() {
  ...
  const { data: team, isLoading: isTeamLoading } = useQuery<TeamDetail>({
    queryKey: ["team", id],
    enabled: !!id,
    queryFn: () => json<TeamDetail>(`/api/teams/${id}`),
    staleTime: 30_000,
  });
  ...
  const {
    ...
    enabled: !!id,
    queryFn: () => json<TeamMemberRow[]>(`/api/teams/${id}/members`),
    staleTime: 30_000,
  });
  ...
  const {
    ...
    queryKey: ["team-member-tracking-metrics", id],
    enabled: !!id,
    queryFn: () =>
      json<TrackingMembersResponse>(
        `/api/tracking-links/members?scope=team&teamId=${encodeURIComponent(
          String(id ?? ""),
        )}`,
      ),
    staleTime: 30_000,
  ...
  const canManageMembers = useMemo(() => {
    ...
    if (team.ownerId && String(team.ownerId) === sessionUserId) {
      return true;
    }
    ...
    const me = (members ?? []).find((member) => {
      ...
      return normalizeEmail(member.email) === sessionUserEmail;
    });

    return me?.status === "active" && me.role === "admin";
  }, [team, members, sessionUserId, sessionUserEmail]);
  ...
  const inviteMutation = useMutation({
    mutationFn: (payload: InvitePayload) =>
      json<{ member: TeamMemberRow }>(`/api/teams/${id}/members`, {
        method: "POST",
        body: JSON.stringify(payload),
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["team-members", id] });
      qc.invalidateQueries({ queryKey: ["team-member-tracking-metrics", id] });
      setInviteOpen(false);
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (args: { memberId: string; body: UpdateBody }) =>
      json<TeamMemberRow>(`/api/teams/${id}/members/${args.memberId}`, {
        method: "PATCH",
  ...
  const deleteMutation = useMutation({
    mutationFn: (memberId: string) =>
      json<{ ok: boolean }>(`/api/teams/${id}/members/${memberId}`, {
        method: "DELETE",
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["team-members", id] });
  ...
  const isLoading = isTeamLoading || isMembersLoading || trackingMetricsLoading;
          ...
          >
            <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <div className="mt-1 text-neutral-400">
                  Manage members, roles, and access for this team
                </div>
              </div>
            ...
            {trackingMetricsError ? (
              <div className="mb-4 rounded-xl border border-warning-500/20 bg-warning-500/10 px-4 py-3 text-[12px] text-warning-200">
                Team performance metrics could not be loaded right now. Member
              ...
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {slice.map((member) => {
                    ...
                    const met =
                      member.userId &&
                      trackingMetricsByUserId.has(member.userId)
                        ...
                        <div className="hidden md:block">
                          <div className={GRID}>
                            ...
                            <div className="text-[13px] text-neutral-400">
                              {prettyDateShort(member.createdAt)}
                            </div>
                        ...
                        <div className="md:hidden">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            ...
                            <MetricChip
                              ...
                              label="Role"
                              value={ROLE_META[member.role]?.label ?? "—"}
                            />
                            <MetricChip
                              icon={<CalendarDays className="h-4 w-4" />}
                              label="Added"
                              value={prettyDateShort(member.createdAt)}
                            />
                          </div>
                        </div>
                      </div>
