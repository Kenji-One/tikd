...
function toHHMMFromDateLocal(d: Date) {
  const hh = String(d.getHours()).padStart(2, "0");
  const mm = String(d.getMinutes()).padStart(2, "0");
  return `${hh}:${mm}`;
}

function guessLocationModeAndFields(locationRaw: string | undefined | null): {
  mode: LocationMode;
  city: string;
  address: string;
  name: string;
  ...
  if (!loc) {
    return { mode: "specific", city: "", address: "", name: "", other: "" };
  }

  const lower = loc.toLowerCase();

  if (lower === "tbd") {
    return { mode: "tbd", city: "", address: "", name: "", other: "" };
  }
  if (lower === "tba") {
    return { mode: "tba", city: "", address: "", name: "", other: "" };
  }
  if (
    lower === "it’s a secret" ||
    lower === "it's a secret" ||
...
export default function EditEventPage() {
  ...
  useEffect(() => {
    ...
    const startISO = event.date ? String(event.date) : "";
    const endISO =
      event.endDate && String(event.endDate).trim()
    ...
    const dateStart =
      startD && !Number.isNaN(startD.getTime()) ? clampToDay(startD) : null;
    const dateEnd =
      endD && !Number.isNaN(endD.getTime()) ? clampToDay(endD) : dateStart;

    const startTime =
      startD && !Number.isNaN(startD.getTime())
        ? toHHMMFromDateLocal(startD)
        : "";
    const endTime =
      endD && !Number.isNaN(endD.getTime()) ? toHHMMFromDateLocal(endD) : "";
    ...
    reset({
      ...
      locationCity: locGuess.city,
      locationAddress: locGuess.address,
      locationName: locGuess.name,
      locationOther: locGuess.other,
      organizationId: orgIdFromEvent || "",
    });
  }, [event, reset, orgIdFromEvent]);

  /* ---------- Lock organizationId from the event ------------------ */
  useEffect(() => {
    if (!orgIdFromEvent) return;
    setValue("organizationId", orgIdFromEvent, { shouldDirty: false });
  }, [orgIdFromEvent, setValue]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/organizations/${orgIdFromEvent}`, { signal: ac.signal })
  ...
  useEffect(() => {
    ...
    fetch(`/api/organizations/${orgIdFromEvent}/members`)
      .then((res) => (res.ok ? res.json() : Promise.reject()))
      .then((data: OrgMember[]) => {
  ...
  // ✅ Live preview: poster-only dropzone (same as creation page)
  ...
  return (
    <main className="relative bg-neutral-950 text-neutral-0 ">
      <style jsx global>{`
        /* ---------------------------------------------
           ✅ Poster upload: make ImageUpload fill height
        ---------------------------------------------- */
        .tikdPosterFill > div {
          height: 100%;
        }
        ...
        /* ---------------------------------------------
           ✅ Clean rounded dashed empty frame
        ---------------------------------------------- */
        ...
        /* ---------------------------------------------------
           ✅ Edit page: kill Steam interactions in preview
        --------------------------------------------------- */
        .tikd-editPreviewCard .tikd-steamCard,
        .tikd-editPreviewCard .tikd-steamCard * {
      ...
      <div className="relative isolate px-4 pt-8 md:py-10 mt-2">
        ...
        <div className="mx-auto max-w-[1232px]">
          ...
          <p className="mt-2 max-w-prose text-sm text-neutral-300">
            Update the essentials, refresh the poster, and save changes.
          </p>
        </div>
      </div>
        ...
        <div className="md:col-span-7 lg:col-span-8 space-y-6">
          <div className="rounded-lg border border-white/10 bg-neutral-950/60 p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                {saved ? (
                  <p className="mt-1 text-xs text-success-300">
            ...
            <div className="space-y-7">
              <div className="space-y-2">
                ...
                <LabelledInput
                  ...
                  placeholder={eventLoading ? "Loading…" : "Enter name"}
                  {...register("title")}
                  size="md"
                  variant="transparent"
            ...
            <div className="mt-3 flex justify-center pb-6">
              ...
              >
                {!hasPoster && !posterDragActive ? (
                  <div className="tikdPosterEmptyFrame" aria-hidden />
                ) : null}
                ...
                <div
                  className={clsx(
                    "tikdPosterFill absolute inset-0 transition-opacity duration-150",
          ...
          <div className="flex flex-wrap justify-between gap-3">
            <Button
              ...
              variant="ghost"
              onClick={() =>
                router.push(`/dashboard/events/${eventId}/summary`)
              }
            >
              Cancel
            </Button>
        ...
        <aside className="md:col-span-5 lg:col-span-4">
          <div className="md:sticky md:top-20 space-y-6">
            <div className="rounded-lg border border-white/10 bg-neutral-950/70 p-5">
              ...
              <div
                ref={previewWrapRef}
                className="tikd-editPreviewCard relative overflow-hidden"
              >
