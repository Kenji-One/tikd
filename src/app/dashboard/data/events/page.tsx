/* ------------------------------------------------------------------ */
/*  src/app/dashboard/data/events/page.tsx                            */
/* ------------------------------------------------------------------ */
"use client";
...
const ROWS: Row[] = Array.from({ length: 10 }).map((_, i) => ({
  id: "#2935",
  name: "Avalon Yacht: Bla Bla",
  income: 892102,
  views: 1000222,
  ticketsSold: 123123123,
...
export default function DataEventsPage() {
  const [sortBy, setSortBy] = useState<SortKey>("dateCreated");
  const [dir, setDir] = useState<SortDir>("desc");

  const [csvOpen, setCsvOpen] = useState(false);
  const [csvEventName, setCsvEventName] = useState<string>("");
  ...
  const columns: DashboardTableColumn<Row>[] = [
    ...
    },
    {
      ...
      header: "Tickets Sold",
      sortable: true,
      sortValue: (r) => r.ticketsSold,
      cell: (r) => (
        <span className="text-neutral-0">
          {r.ticketsSold.toLocaleString("en-US")}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "DJ Songs",
      sortable: true,
      sortValue: (r) => r.djSongs,
      cell: (r) => <span className="text-neutral-0">{r.djSongs}</span>,
    },
    {
      key: "actions",
      header: "",
