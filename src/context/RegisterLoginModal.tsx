...
export default function RegisterLoginModal({
  ...
  const { login, register, error, fieldErrors, clearErrors, loading } =
    useAuth();
  ...
  // 🔽 add this helper (same rules as API)
  ...
  const canSubmit = (() => {
    ...
    if (mode === "register")
      return (
        ...
        Boolean(password) &&
        agreeTerms &&
        !passwordStrengthError // 🔒 block submit if not strong
      );
  ...
  // ↓ place this just after handleSubmit()
  ...
  // ↓ add alongside your other helpers/state
  ...
  /* helpers to mark blur/focus without changing visuals */
  ...
  const resetFormState = (): void => {
    ...
    // ▼ NEW: clear "required" borders/messages after tab change
  ...
  const handleTabChange = (next: "register" | "login"): void => {
    if (isAllEmpty(mode)) {
      resetFormState();
    }
    setMode(next);
  ...
  const syncPw = (e: React.FormEvent<HTMLInputElement>) =>
          ...
          {(mode === "login" || mode === "register") && (
            <div className="relative mb-10 flex">
              {["register", "login"].map((t) => (
                /* ★ added type="button" so tab click doesn’t submit the form */
                <button
                  type="button"
                  key={t}
                  onClick={() => handleTabChange(t as "register" | "login")}
                  className={clsx(
          ...
          >
            ...
            {mode === "forgot" && (
              <LabelledInput
                ...
                /* show red border only when NOT focused */
                error={
                  focused === "email"
                    ? null
                    : uiErrors.email || fieldErrors.email || null
                }
              />
            )}
            {mode === "verify" && (
              ...
              <>
                <LabelledInput
                  ...
                  error={
                    focused === "username"
                      ? null
                      : uiErrors.username || fieldErrors.username || null
                  }
                />
                {mode === "register" && (
                  <LabelledInput
                ...
                <div>
                  <LabelledInput
                    ...
                    /* login: show auth error under password; during focus, revert to default border */
                    error={
                      focused === "password"
                        ? null
                        : mode === "login"
                    ...
                    endAdornment={
                      <button
                        ...
                        className="text-neutral-400 hover:text-white focus:outline-none"
                      >
                        {/* eye / eye-off icon (stroke follows currentColor) */}
                        {showPassword ? (
                          <svg
                            width="20"
                            height="20"
            ...
            {/* keep global error message (non-login modes). Border colors remain unchanged. */}
            {error && mode !== "login" && (
