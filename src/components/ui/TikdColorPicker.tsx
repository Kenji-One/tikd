...
type Props = {
  ...
  /** Optional swatch palette */
  swatches?: string[];

  /** Layout mode */
  mode?: "full" | "compact";

  /** If true, render ONLY the popover picker (no inline trigger UI). */
  popoverOnly?: boolean;
  ...
  /** Where to anchor the popover (client coords) when opened externally */
...
export default function TikdColorPicker({
  ...
  useEffect(() => {
    ...
    if (
      ...
      setClickPt({ x: anchorPoint.x, y: anchorPoint.y });
    }
  ...
  const Swatches = swatches.length ? (
    <div className="mt-3 grid grid-cols-12 gap-2">
      {swatches.map((c) => {
        const cc = normalizeHex(c);
        const active = normalizeHex(displayHex) === cc;
        return (
          <button
            key={cc}
            type="button"
            ...
            className={clsx(
              ...
              active
                ? "border-white/40 ring-2 ring-primary-500/50"
                : "border-white/10",
        ...
        <>
            ...
            >
              <span
                ...
                style={{
                  background:
                    displayHex &&
                    /^#([0-9A-F]{6})$/i.test(normalizeHex(displayHex))
                      ? normalizeHex(displayHex)
                      : "rgba(255,255,255,0.10)",
                }}
              />
              <span className="font-mono text-[11px] text-neutral-200">
                {normalizeHex(displayHex) || "DEFAULT"}
              </span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
            ...
            <button
              type="button"
              onClick={resetToDefault}
              className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-[12px] font-semibold text-neutral-0 hover:bg-white/10 transition"
            >
              Reset
            </button>
          </div>
