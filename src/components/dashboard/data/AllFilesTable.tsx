...
function FileBadge({ kind }: { kind: Row["kind"] }) {
  const label = kind === "sketch" ? "SKT" : "PDF";

  return (
    <div
      className={clsx(
        "grid h-7 w-7 shrink-0 place-items-center rounded-md bg-[#FF5A2A]",
      )}
      aria-hidden
    >
...
function MobileFileCard({ row }: { row: Row }) {
  return (
    <div className="rounded-xl border border-neutral-700 bg-neutral-900 p-4 shadow-[0_18px_56px_rgba(0,0,0,0.36),inset_0_1px_0_rgba(255,255,255,0.04)]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 text-xs text-neutral-300">
            {row.kind === "sketch" ? "Sketch file" : "PDF document"}
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-lg border border-white/8 bg-black/20 px-3 py-2">
          ...
          <div className="mt-1 text-sm text-neutral-0">{row.lastEdit}</div>
        </div>
    ...
    <div>
      ...
      <div className="space-y-3 md:hidden">
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={() => toggleSort("lastEdit")}
            className="rounded-full border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-xs font-medium text-neutral-100"
          >
            Last Edit
          </button>
          <button
        ...
        {sorted.map((row) => (
          <MobileFileCard key={row.id} row={row} />
        ))}
      </div>
