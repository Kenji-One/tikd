...
function buildEventRedirect(eventId: string): string {
  return `/dashboard/events/${eventId}/summary`;
}
...
export function buildEventInviteAcceptUrl(rawToken: string): string {
  return `${appBaseUrl()}/invite/event/${encodeURIComponent(rawToken)}`;
}
...
async function findInviterName(
  ...
  if (!inviter) return "An event admin";

  const full = `${inviter.firstName ?? ""} ${inviter.lastName ?? ""}`.trim();
  return full || inviter.username || inviter.email || "An event admin";
}
...
async function loadInviteByRawToken(rawToken: string) {
  ...
  return EventTeam.findOne({
    ...
    .select(
      "_id eventId email role status temporaryAccess expiresAt invitedBy inviteExpiresAt",
    )
    .lean<{
      _id: Types.ObjectId;
...
export async function getEventInvitePreviewByToken(
  rawToken: string,
  ...
  return {
    ...
    redirectTo: buildEventRedirect(String(event._id)),
  };
}

export async function acceptEventInviteByToken(input: {
  rawToken: string;
  userId: string;
  email: string;
}): Promise<
...
> {
  ...
  const updated = await EventTeam.findOneAndUpdate(
    ...
    .select("_id eventId")
    .lean<{ _id: Types.ObjectId; eventId: Types.ObjectId } | null>();
  ...
  return {
    ok: true,
    redirectTo: buildEventRedirect(String(updated.eventId)),
  };
}

export async function sendEventInviteEmail(input: {
  to: string;
  eventTitle: string;
  roleName: string;
  inviterName?: string;
  ...
  try {
    const acceptUrl = buildEventInviteAcceptUrl(input.rawToken);

    const safeEventTitle = escapeHtml(input.eventTitle);
    const safeRoleName = escapeHtml(input.roleName);
    const safeInviterName = escapeHtml(input.inviterName || "An event admin");
    ...
    await sendMail({
      to: input.to,
      subject: `You’ve been invited to join ${input.eventTitle} on Tixsy`,
      html: `
          ...
          <div style="max-width:640px;margin:0 auto;border:1px solid rgba(255,255,255,0.08);border-radius:24px;overflow:hidden;background:
            ...
            <div style="padding:28px 28px 18px;">
              ...
              <p style="margin:0 0 16px;font-size:15px;line-height:1.7;color:#B8C0D9;">
                <strong style="color:#FFFFFF;">${safeInviterName}</strong> invited you to work on
                <strong style="color:#FFFFFF;">${safeEventTitle}</strong> on Tixsy.
              </p>
